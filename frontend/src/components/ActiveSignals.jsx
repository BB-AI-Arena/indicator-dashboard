...
function SignalCard({ title, signal }) {
  ...
  const contract = signal.preferred_option_contract || {}
  return (
    <section className="card border border-emerald-500/20 p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div><span className="text-slate-500">Entry</span><div className="font-medium text-white">{signal.entry?.condition || money(signal.entry?.price)}</div><div className="text-xs text-slate-500">Zone: {signal.entry?.acceptable_zone || 'Exact trigger only'}</div></div>
        ...
        <div><span className="text-slate-500">Expires</span><div className="font-medium text-white">{signal.valid_until ? new Date(signal.valid_until).toLocaleTimeString() : 'Unavailable'}</div><div className="text-xs text-slate-500">Hold: {signal.expected_holding_window}</div></div>
      </div>
      ...
      <div className="mt-3 text-xs text-slate-500">Option: {contract.contract || 'Unavailable'} · max premium {money(contract.maximum_acceptable_premium)}</div>
    </section>
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="card p-4"><div className="section-kicker">Market state</div><div className="mt-2 text-lg font-semibold text-white">{session?.session_state || 'Loading'}</div><div className="mt-1 text-xs text-slate-500">Regime: {payload?.market_regime || 'Unavailable'} · {payload?.data_health || 'Checking data health'}</div></div>
      ...
      {signals.length > 0 && <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        ...
        {selectedSignal && <section className="card p-5"><div className="section-kicker">Signal detail</div><div className="mt-1 flex items-center justify-between"><h2 className="text-xl font-semibold text-white">{selectedSignal.ticker} {selectedSignal.direction}</h2><span className="status-chip status-chip-ok">{selectedSignal.state}</span></div><div className="mt-4 space-y-3 text-sm"><div><span className="text-slate-500">Setup</span><div className="text-white">{selectedSignal.setup_type} · {selectedSignal.confidence || 'Unavailable'} confidence</div></div><div><span className="text-slate-500">Exact entry</span><div className="text-white">{selectedSignal.entry?.condition}</div><div className="text-xs text-slate-500">Acceptable zone: {selectedSignal.entry?.acceptable_zone || 'Exact trigger only'}</div></div><div><span className="text-slate-500">Maximum chase</span><div className="text-white">{money(selectedSignal.maximum_chase_underlying || selectedSignal.entry?.maximum_chase_price)}</div></div><div><span className="text-slate-500">Invalidation</span><div className="text-rose-300">{selectedSignal.invalidation?.condition}</div></div><div><span className="text-slate-500">Targets</span><div className="text-emerald-300">{(selectedSignal.targets || []).slice(0, 2).map(row => money(row.price)).join(' / ') || 'Unavailable'}</div></div><div><span className="text-slate-500">Preferred contract</span><div className="text-white">{selectedSignal.preferred_option_contract?.contract || 'Unavailable'}</div><div className="text-xs text-slate-500">{selectedSignal.preferred_option_contract?.expiration || 'Expiration unavailable'} · strike {selectedSignal.preferred_option_contract?.strike || 'unavailable'} · {selectedSignal.preferred_option_contract?.type || 'type unavailable'}</div><div className="text-xs text-slate-500">Last {money(selectedSignal.preferred_option_contract?.last)} · bid {money(selectedSignal.preferred_option_contract?.bid)} · ask {money(selectedSignal.preferred_option_contract?.ask)} · midpoint {money(selectedSignal.preferred_option_contract?.midpoint)}</div><div className="text-xs text-slate-500">Maximum premium: {money(selectedSignal.preferred_option_contract?.maximum_acceptable_premium)}</div></div><div><span className="text-slate-500">Validation</span><div className="text-white">AI: {selectedSignal.ai_validation?.status || 'Unavailable'} · data: {selectedSignal.data_freshness?.latest_scan_at ? new Date(selectedSignal.data_freshness.latest_scan_at).toLocaleTimeString() : 'Unavailable'}</div></div><div><span className="text-slate-500">Historical match</span><div className="text-white">{selectedSignal.historical_match?.raw_hit_rate ?? selectedSignal.historical_match?.raw_success_rate ?? 'Unavailable'} · sample {selectedSignal.historical_match?.sample_size ?? selectedSignal.historical_match?.occurrence_count ?? 'Unavailable'}</div></div><div><span className="text-slate-500">Primary conflict</span><div className="text-amber-200">{selectedSignal.primary_conflict || 'None recorded'}</div></div><div className="border-t border-slate-800 pt-3"><strong className="text-white">Next action:</strong> <span className="text-slate-300">{selectedSignal.next_action}</span></div></div></section>}
      </div>}
